...
class federateagent:
    ...
    def run_helics_setup(self,comm_type="zmq",broker_ip="127.0.0.1", broker_num=0,broker_port=32000):
        """
        ...
        fedinitstring = "--federates=1  --brokerport={} --broker_address={} ".format(broker_port, broker_ip)
        ...
        if True:
            self.cfed = h.helicsCreateValueFederate(self.name_helics ,fedinfo)
        else:
        ...
        if True:
            ...
            for x in self.helics['publication_topic']:
                self.pub[x] = h.helicsFederateRegisterGlobalTypePublication(self.cfed, x , "double", "")
            self.sub = {}
            for x in self.helics['subscription_topic']:
                self.sub[x] = h.helicsFederateRegisterSubscription(self.cfed,x ,"double")
            self.ends = {}
            ...
            self.sub = {}
            for x in self.helics['subscription_topic']:
                self.sub[x] = h.helicsFederateRegisterSubscription(self.cfed,x ,"")
            self.ends = {}
            for x in self.helics['endpoints']:
                self.ends[x] =h.helicsFederateRegisterGlobalEndpoint(fed=self.cfed, name=x)        
    ...
    def helics_get_all(self):
        ...
        for key in self.sub.keys():
            ...
            # print("Key : ",x)
            # if True:
            # msg = h.helicsInputGetRawValue(x)
            msg = h.helicsInputGetDouble(x)
            # value_recv = h.helicsInputGetDouble(x)
            # print("value_recv ",value_recv)
    ...
    def helics_broadcast(self,topic,message):
        ...
        # print(topic, message, "publish")
        h.helicsPublicationPublishDouble(self.pub[topic], float(message))
        # h.helicsPublicationPublishString(topic, str(msg_dict))
