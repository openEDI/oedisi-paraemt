    ...
    def LargeSysGenerator(self, ItfcBus, r, c):
        ...
        for i in range(r):
            for j in range(c):
                ...
                pfd.bus_name = np.append(pfd.bus_name, self.bus_name)
                pfd.bus_num = np.append(pfd.bus_num, self.bus_num + dbusN * (k - 1))
                ...
                if len(self.load_bus) > 0:
                    ...
                    pfd.load_bus = np.append(pfd.load_bus, self.load_bus + dbusN * (k - 1))
                    pfd.load_id = np.append(pfd.load_id, self.load_id)

                # ibr
                if len(self.ibr_bus) > 0:
                    pfd.ibr_MVA_base = np.append(pfd.ibr_MVA_base, self.ibr_MVA_base)
                    pfd.ibr_MW = np.append(pfd.ibr_MW, self.ibr_MW)
                    pfd.ibr_Mvar = np.append(pfd.ibr_Mvar, self.ibr_Mvar)
                    pfd.ibr_bus = np.append(pfd.ibr_bus, self.ibr_bus + dbusN * (k - 1))
                    pfd.ibr_id = np.append(pfd.ibr_id, self.ibr_id)
                ...
                pfd.gen_bus = np.append(pfd.gen_bus, self.gen_bus + dbusN * (k - 1))
                ...
                if len(self.line_from) > 0:
                    ...
                    pfd.line_from = np.append(pfd.line_from, self.line_from + dbusN * (k - 1))
                    pfd.line_id = np.append(pfd.line_id, self.line_id)
                    pfd.line_to = np.append(pfd.line_to, self.line_to + dbusN * (k - 1))
                ...
                if len(self.xfmr_from) > 0:
                    ...
                    pfd.xfmr_from = np.append(pfd.xfmr_from, self.xfmr_from + dbusN * (k - 1))
                    pfd.xfmr_id = np.append(pfd.xfmr_id, self.xfmr_id)
                    pfd.xfmr_k = np.append(pfd.xfmr_k, self.xfmr_k)
                    pfd.xfmr_to = np.append(pfd.xfmr_to, self.xfmr_to + dbusN * (k - 1))

                # shunt
                if len(self.shnt_bus) > 0:
                    pfd.shnt_gb = np.append(pfd.shnt_gb, self.shnt_gb)
                    pfd.shnt_bus = np.append(pfd.shnt_bus, self.shnt_bus + dbusN * (k - 1))
                    pfd.shnt_id = np.append(pfd.shnt_id, self.shnt_id)
                
                # switched shunt
                if len(self.shnt_sw_bus) > 0:
                    pfd.shnt_sw_gb = np.append(pfd.shnt_sw_gb, self.shnt_sw_gb)
                    pfd.shnt_sw_bus = np.append(pfd.shnt_sw_bus, self.shnt_sw_bus + dbusN * (k - 1))
        ...
        for i in range(len(branch_to_add)):
            ...
            FromB_B = np.where(pfd.bus_num==FromB)[0][0]
            ToB_B = np.where(pfd.bus_num==ToB)[0][0]

            FromB_G = []
            ToB_G = []
            if FromB in pfd.gen_bus:
                FromB_G = np.array(np.where(pfd.gen_bus==FromB)[0])

                idx0 = []
                for jj in range(len(FromB_G)):
                    if pfd.gen_MW[FromB_G[jj]]<1e-5:
                        idx0.append(jj)
                lenidx0 = len(idx0)
                for jj in range(len(idx0)):
                    FromB_G = np.delete(FromB_G, idx0[lenidx0-jj-1])
                
                PG_From = 0
                for geni in FromB_G:
                    PG_From = PG_From + pfd.gen_MW[geni]
            else:
                print('Error: Interfacing bus should be generator bus.\n')

            if ToB in pfd.gen_bus:
                ToB_G = np.array(np.where(pfd.gen_bus==ToB)[0])

                idx0 = []
                for jj in range(len(ToB_G)):
                    if pfd.gen_MW[ToB_G[jj]]<1e-5:
                        idx0.append(jj)
                lenidx0 = len(idx0)
                for jj in range(len(idx0)):
                    ToB_G = np.delete(ToB_G, idx0[lenidx0-jj-1])

                PG_To = 0
                for geni in ToB_G:
                    PG_To = PG_To + pfd.gen_MW[geni]
            else:
                print('Error: Interfacing bus should be generator bus.\n')

            tempP = min(PG_From / 3, PG_To / 3, LinePmax) / pfd.basemva
            tempX = pfd.bus_Vm[FromB_B - 1] * pfd.bus_Vm[ToB_B - 1] * math.sin(
                abs(pfd.bus_Va[FromB_B - 1] - pfd.bus_Va[ToB_B - 1])) / tempP
            ...
            chg = 1e-5
            # add the branch
            if abs(pfd.bus_basekV[FromB_B - 1] - pfd.bus_basekV[ToB_B - 1]) < 1e-5:  # kV level is the same, this is a line
                tempr = tempX / 10.0
                ...
                tempr = tempX / 100.0
                if pfd.bus_basekV[FromB_B - 1] > pfd.bus_basekV[ToB_B - 1]:
            ...
            # actual line flow
            Vf = complex(pfd.bus_Vm[FromB_B - 1] * math.cos(pfd.bus_Va[FromB_B - 1]),
                         pfd.bus_Vm[FromB_B - 1] * math.sin(pfd.bus_Va[FromB_B - 1]))
            Vt = complex(pfd.bus_Vm[ToB_B - 1] * math.cos(pfd.bus_Va[ToB_B - 1]),
                         pfd.bus_Vm[ToB_B - 1] * math.sin(pfd.bus_Va[ToB_B - 1]))
            ...
            if np.real(tempS_From) > 0:  # need to add generation
                for j in range(len(FromB_G)):
                    pfd.gen_MW[FromB_G[j]] = pfd.gen_MW[FromB_G[j]] + np.real(tempS_From) * pfd.basemva * pfd.gen_MW[FromB_G[j]] / PG_From  # not run into during testing
            ...
            if np.real(tempS_To) > 0:  # need to add generation
                for j in range(len(ToB_G)):
                    pfd.gen_MW[ToB_G[j]] = pfd.gen_MW[ToB_G[j]] + np.real(tempS_To) * pfd.basemva * pfd.gen_MW[ToB_G[j]] / PG_To  # not run into during testing
