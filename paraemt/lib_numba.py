...
def numba_InitNet(
    ...
    if loadmodel_option == 1:
        nentries = 12 * len(line_from) + 12 * len(xfmr_from) + 3 * len(load_bus) + 3 * len(shnt_bus) + 3 * len(
            shnt_sw_bus) + 12*ibr_epri_n
    else:
        nentries = 12*len(line_from) + 12*len(xfmr_from) + 3*len(shnt_bus) + 3 * len(shnt_sw_bus) + 12*ibr_epri_n
    #
    ...
    if loadmodel_option == 1:
        nbranch = 9*len(line_from) + 3*len(xfmr_from) + 3*len(load_bus) + 3*len(shnt_bus) + 3*len(shnt_sw_bus) + 6*ibr_epri_n
    else:
        nbranch = 9 * len(line_from) + 3 * len(xfmr_from) + 3 * len(shnt_bus) + 3 * len(shnt_sw_bus) + 6*ibr_epri_n
    ...
    for i in range(len(line_from)):
        ...
        if X>0:
            ...
            ## question, why not add Rp here?
            CL = - 1 / X / ws
            Req = R + ts / 2 / CL
            icf = (2*R*CL - ts) / (2*R*CL + ts)
            # Gv1 = -2*CL / (2*R*CL + ts) ## Min added
...
def numba_predictX(
    ...
    for i in numba.prange(gen_range[0], gen_range[1]):
        ...
        pv_u_d_1[i] = -pv_psyq_1[i] * pv_w_1[i] / ws # -x_pv_1[idx + 13] * x_pv_1[idx + 1]
        pv_u_q_1[i] = pv_psyd_1[i] * pv_w_1[i] / ws # x_pv_1[idx + 12] * x_pv_1[idx + 1]

        pv_i_d_1[0,i] = neg_id
        pv_i_d_1[1,i] = pv_ifd_1[i]
        pv_i_d_1[2,i] = pv_i1d_1[i]

        pv_i_q_1[0,i] = neg_iq
        pv_i_q_1[1,i] = pv_i1q_1[i]
        pv_i_q_1[2,i] = pv_i2q_1[i]
    ...
    if xlen == 1:
        ...
        for i in numba.prange(gen_range[0], gen_range[1]):
            ...
            pv_w_2 = x_pv_2[idx + 1]
            pv_id_2 = x_pv_2[idx + 2]
            pv_iq_2 = x_pv_2[idx + 3]

            pv_psyd_2 = x_pv_2[idx + 10]
            pv_psyq_2 = x_pv_2[idx + 11]

            alt_idx = i * exc_sexs_odr + exc_sexs_xi_st
            pv_EFD_2 = x_pv_2[alt_idx + 1]
            ...
            if xlen == 2:
                ...
                pd_w[i] = 2.0 * pv_w_1[i] - pv_w_2
                pd_id[i] = 2.0 * pv_id_1[i] - pv_id_2
                pd_iq[i] = 2.0 * pv_iq_1[i] - pv_iq_2
                pd_EFD[i] = 2.0 * pv_EFD_1[i] - pv_EFD_2
                pd_u_d[i] = 2.0 * pv_u_d_1[i] - pv_u_d_2
                pd_u_q[i] = 2.0 * pv_u_q_1[i] - pv_u_q_2

            else: # three-point prediction

                pv_w_3 = x_pv_3[idx + 1]
                pv_id_3 = x_pv_3[idx + 2]
                pv_iq_3 = x_pv_3[idx + 3]

                pv_psyd_3 = x_pv_3[idx + 10]
                pv_psyq_3 = x_pv_3[idx + 11]

                alt_idx = i * exc_sexs_odr + exc_sexs_xi_st
                pv_EFD_3 = x_pv_3[alt_idx + 1]

                pv_u_d_3 = -pv_psyq_3 * pv_w_3 / ws
                pv_u_q_3 = pv_psyd_3 * pv_w_3 / ws

                pd_w[i] = 1.25 * pv_w_1[i] + 0.5 * pv_w_2 - 0.75 * pv_w_3
                pd_id[i] = 1.25 * pv_id_1[i] + 0.5 * pv_id_2 - 0.75 * pv_id_3
                pd_iq[i] = 1.25 * pv_iq_1[i] + 0.5 * pv_iq_2 - 0.75 * pv_iq_3
                pd_EFD[i] = 1.25 * pv_EFD_1[i] + 0.5 * pv_EFD_2 - 0.75 * pv_EFD_3
                pd_u_d[i] = 1.25 * pv_u_d_1[i] + 0.5 * pv_u_d_2 - 0.75 * pv_u_d_3
                pd_u_q[i] = 1.25 * pv_u_q_1[i] + 0.5 * pv_u_q_2 - 0.75 * pv_u_q_3
...
def numba_updateIg(
    ...
    for i in numba.prange(gen_range[0], gen_range[1]):
        ...
        pv_ed = x_pv_1[i * gen_genrou_odr + 8]
        pv_eq = x_pv_1[i * gen_genrou_odr + 9]
        ...
        coef = base_Is[i] / (Init_net_IbaseA[genbus_idx] * 1000.0)
        Isg[3*i] = res[0] * coef
        Isg[3*i + 1] = res[1] * coef
        Isg[3*i + 2] = res[2] * coef

        Ias_n[genbus_idx] += Isg[3 * i]
        Ibs_n[genbus_idx] += Isg[3 * i + 1]
        Ics_n[genbus_idx] += Isg[3 * i + 2]
...
def numba_updateIibr(
    ...
    for i in numba.prange(ibr_range[0],ibr_range[1]):
...
def numba_updateIl(
    ...
    for i in numba.prange(load_range[0],load_range[1]):

        busi_idx = np.where(bus_num == load_bus[i])[0][0]

        ZL_mag = x_load_pv_1[i * load_odr + 0]
        ZL_ang = x_load_pv_1[i * load_odr + 1]

        Vmag = x_bus_pv_1[busi_idx * bus_odr + 3]
        Vang = x_bus_pv_1[busi_idx * bus_odr + 1]

        if tn * ts > t_release_f:
            w = x_bus_pv_1[busi_idx * bus_odr + 2] * ws
        else:
            w = ws
        ...
        Vanga = Vang + w * ts
        Il[busi_idx] = -Imag * np.cos(Vanga - ZL_ang)

        Vangb = Vanga - 2 * np.pi / 3 # + w * ts
        Il[busi_idx + nbus] = -Imag * np.cos(Vangb - ZL_ang)

        Vangc = Vanga + 2 * np.pi / 3 # + w * ts
        Il[busi_idx + 2 * nbus] = -Imag * np.cos(Vangc - ZL_ang)

        Ild[3 * i] = -Imag * np.cos(Vanga - ZL_ang)
        Ild[3 * i + 1] = -Imag * np.cos(Vangb - ZL_ang)
        Ild[3 * i + 2] = -Imag * np.cos(Vangc - ZL_ang)
...
def numba_BusMea(
    ...
    for i in numba.prange(bus_range[0], bus_range[1]):
...
def numba_updateX(
    ...
    for i in numba.prange(gen_range[0], gen_range[1]):
        ...
        # ## tried on 12/4/2023 for speeding up initialization
        # if tn*ts < 5:
        #     nx_w = pv_w_1[i] + (ws*(pv_pm/(pv_w_1[i]/ws) - nx_pe) - 10*gen_D[i] * (pv_w_1[i] - ws)) / gen_H[i]/10 / 2.0 * ts


        # ## replay recorded rotor speed
        # if playback_voltphasor==True:
        #     nx_w = tsat_gen_omg[tn][tsat_gen_maci[i]+1]*2*np.pi
        #     # # interpolation (not working well for signals from TSAT at large time step)
        #     # iddx = np.where(tsat_t>=tn*ts)[0]
        #     # iddx = iddx[0]-1
        #     # alfa = (tn*ts-tsat_t[iddx]) / (tsat_t[iddx+1] - tsat_t[iddx])

        #     # nx_w = (1-alfa)*tsat_gen_omg[iddx][tsat_gen_maci[i]]*2*np.pi + alfa*tsat_gen_omg[iddx+1][tsat_gen_maci[i]]*2*np.pi
        ...
        if len(idx_pss1)==0:
            ...
            pv_vs = x_pv_1[pss_ieeest_odr * idx_pss + pss_ieeest_xi_st + 9]
            dvs = (vs - pv_vs)/ts
        ...
        nx_v1 = pv_v1 + ((vref_n - exc_input_vt - pv_v1 + vs)/exc_sexs_TB[i] + exc_sexs_TA[i]/exc_sexs_TB[i]*(dvref + dvs - exc_input_dvt)) * ts
...
def numba_updateXibr(
    ...
    for i in numba.prange(ibr_range[0], ibr_range[1]):
        ...
        idx = i * Init_ibr_N
        regca_s0_1 = x_ibr_pv_1[idx + 0]
        regca_s1_1 = x_ibr_pv_1[idx + 1]
        regca_s2_1 = x_ibr_pv_1[idx + 2]
        regca_Vmp_1 = x_ibr_pv_1[idx + 3]
        regca_Vap_1 = x_ibr_pv_1[idx + 4]
        regca_i1_1 = x_ibr_pv_1[idx + 5]
        regca_i2_1 = x_ibr_pv_1[idx + 6]
        regca_ip2rr = x_ibr_pv_1[idx + 7]

        reecb_s0_1 = x_ibr_pv_1[idx + 8]
        reecb_s1_1 = x_ibr_pv_1[idx + 9]
        reecb_s2_1 = x_ibr_pv_1[idx + 10]
        reecb_s3_1 = x_ibr_pv_1[idx + 11]
        reecb_s4_1 = x_ibr_pv_1[idx + 12]
        reecb_s5_1 = x_ibr_pv_1[idx + 13]
        reecb_Ipcmd_1 = x_ibr_pv_1[idx + 14]
        reecb_Iqcmd_1 = x_ibr_pv_1[idx + 15]
        reecb_Pref_1 = x_ibr_pv_1[idx + 16]
        reecb_Qext_1 = x_ibr_pv_1[idx + 17]
        reecb_q2vPI_1 = x_ibr_pv_1[idx + 18]
        reecb_v2iPI_1 = x_ibr_pv_1[idx + 19]

        repca_s0_1 = x_ibr_pv_1[idx + 20]
        repca_s1_1 = x_ibr_pv_1[idx + 21]
        repca_s2_1 = x_ibr_pv_1[idx + 22]
        repca_s3_1 = x_ibr_pv_1[idx + 23]
        repca_s4_1 = x_ibr_pv_1[idx + 24]
        repca_s5_1 = x_ibr_pv_1[idx + 25]
        repca_s6_1 = x_ibr_pv_1[idx + 26]
        repca_Vref_1 = x_ibr_pv_1[idx + 27]
        repca_Qref_1 = x_ibr_pv_1[idx + 28]
        repca_Freq_ref_1 = x_ibr_pv_1[idx + 29]
        repca_Plant_pref_1 = x_ibr_pv_1[idx + 30]
        repca_LineMW_1 = x_ibr_pv_1[idx + 31]
        repca_LineMvar_1 = x_ibr_pv_1[idx + 32]
        repca_LineMVA_1 = x_ibr_pv_1[idx + 33]
        repca_QVdbout_1 = x_ibr_pv_1[idx + 34]
        repca_fdbout_1 = x_ibr_pv_1[idx + 35]
        repca_vq2qPI_1 = x_ibr_pv_1[idx + 36]
        repca_p2pPI_1 = x_ibr_pv_1[idx + 37]
        ...
        Sbranch = np.complex128(repca_LineMW_1, repca_LineMvar_1)
        Vcom = np.complex128(Vm * np.cos(Va), Vm * np.sin(Va))
        Ibranch = np.conj(Sbranch / Vcom)
        V1_in1 = np.abs(Vcom - np.complex128(ibr_repca_Rc[i], ibr_repca_Xc[i]) * Ibranch)
        ...
        nx_repca_LineMVA = np.abs(np.complex128(pe, qe))
...
def numba_updateXl(
    ...
    for i in numba.prange(load_range[0], load_range[1]):
        ...
        busi = np.where(bus_num == load_bus[i])[0][0]

        # ZL_mag = x_load_pv_1[i * load_odr + 0]
        # ZL_ang = x_load_pv_1[i * load_odr + 1]
        # # vt_tn = x_bus_pv_1[busi * bus_odr + 3]

        # # check and update Zload (without update, const Z load is used)
        # x_load_nx[i * load_odr + 0] = ZL_mag
        # x_load_nx[i * load_odr + 1] = ZL_ang
        # # x_load_nx[i * load_odr + 2] = Stemp.real
        # # x_load_nx[i * load_odr + 3] = Stemp.imag
        ...
        x_load_nx[i * load_odr + 2] = pe
        x_load_nx[i * load_odr + 3] = qe
...
def numba_update_vpn0(
    ...
    for i in range(nbus):
        vabc_phasor = np.array([vm[i] * np.exp(1j * va[i]),
                                vm[i+nbus] * np.exp(1j * va[i+nbus]),
                                vm[i+2*nbus] * np.exp(1j * va[i+2*nbus]),
                                ])
...
def numba_update_ipn0(
    ...
    for i in range(nbus):
        iabc_phasor = np.array([im[i] * np.exp(1j * ia[i]),
                                im[i+nbus] * np.exp(1j * ia[i+nbus]),
                                im[i+2*nbus] * np.exp(1j * ia[i+2*nbus]),
                                ])
