...
def initialize_emt(
    ...
    if systemN == 1:
        ...
        pfd_name = (
            "pfd_3bus_" + str(N_row) + "_" + str(N_col) + "_newcase" + str(systemN - 17)
        )
    elif systemN == 21:
    ...
    if systemN == 1:
        ...
        fdyd = os.path.join(
            "2area_psse", "OneGov", "TGOV1", "Benchmark_twoarea_ibrepri.xlsx"
        )
    elif systemN == 9:
        fdyd = os.path.join("2area_psse", "OneGov", "TGOV1", "Benchmark_twoarea.xlsx")
    ...
    if len(fnwp) > 0:
        ...
        for i in range(emtzones_n):
            ...
            for j in range(zbuses_n):
                zbuses = np.append(
                    zbuses, int(nwp_data.cell_value(bdrybus_n + 3 + j, i + 1))
                )
    ...
    if len(emt_zones) > 0 and record4cosim == False:
        del_gen_idx, del_ibr_idx, emt_buses = pfd.extract_emt_area(
            [emt_zones[EMT_N - 1]]
        )
...
def initialize_from_snp(input_snp, netMod):
    with open(input_snp, "rb") as f:
        ...
        # emt.v = {}
        # emt.v[0] = x
        ini.Init_net_G0 = sp.coo_matrix(
            (ini.Init_net_G0_data, (ini.Init_net_G0_rows, ini.Init_net_G0_cols)),
...
def initialize_bus_fault(emt, mode = 'inv'):
    ...
    ini_ft.InitNet(pfd, dyd, emt.ts, emt.loadmodel_option)
    ...
    # Revised code, delete anything related to ft_idx, by Min
    N1 = len(pfd.bus_num)
    ...
    #======= Choice 1
    # pfd_pf = deepcopy(pfd)
    # pfd_pf.line_RX = np.delete(pfd_pf.line_RX, emt.fault_line_idx)
    # pfd_pf.line_chg = np.delete(pfd_pf.line_chg, emt.fault_line_idx)
    # pfd_pf.line_from = np.delete(pfd_pf.line_from, emt.fault_line_idx)
    # # pfd_pf.line_id = np.delete(pfd_pf.line_id, emt.fault_line_idx)
    # pfd_pf.line_to = np.delete(pfd_pf.line_to, emt.fault_line_idx)
    # ini_pf = Initialize(pfd_pf, dyd)
    # ini_pf.InitNet(pfd_pf, dyd, emt.ts, emt.loadmodel_option)
    ...
    #                                      ).tolil()
    # emt.bus_del_ind=np.array(range(9*emt.fault_line_idx , 9*(emt.fault_line_idx+1))) # line is put at the begining
    #======= Choice 2
    pfd_pf = deepcopy(pfd)
...
def initialize_fault(emt, mode="inv"):
    ...
    Yft[6, 6] = (
        1 / ((rt + Lt * 1j) * emt.fault_dist)
        + 1 / ((rt + Lt * 1j) * (1 - emt.fault_dist))
        + 1 / r1g
        + 1 / r12
        + 1 / r13
    )
    ...
    Yftr = Yft[np.ix_(range1, range1)] - np.matmul(
        np.matmul(
            Yft[np.ix_(range1, range2)], np.linalg.inv(Yft[np.ix_(range2, range2)])
        ),
        Yft[np.ix_(range2, range1)],
    )
    ...
    ini_ft.Init_net_G0 = sp.coo_matrix(
        (ini_ft.Init_net_G0_data, (ini_ft.Init_net_G0_rows, ini_ft.Init_net_G0_cols)),
        shape=(ini_ft.Init_net_N, ini_ft.Init_net_N),
    ).tolil()
    ...
    for i in range(len(ini.Init_net_coe0)):
        ...
        if (
            np.abs(ini.Init_net_coe0[i, 0] - ini_ft.Init_net_coe0[j, 0])
            + np.abs(ini.Init_net_coe0[i, 1] - ini_ft.Init_net_coe0[j, 1])
            < 1e-5
        ):  # From and To bus
            pass
        else:
            ini_ft.Init_net_coe0 = np.insert(
                ini_ft.Init_net_coe0, j, np.zeros((1, 9)), 0
            )  # Give 0 values first
            indx_ft.append(j)
        j += 1
    ...
    m = len(PF_Ln_add) - n
    ft_idx = ini_ft.resize_G0_data(
        n + 4 * m
    )  # expand to 66 with 0 for non-existing elements
    ...
    for i in range(len(PF_Ln_add)):
        ...
        if Tobus == -1:
            if np.abs(C) < 1e-10:
                ini_ft.addtoG0(
                    ft_idx, Frombus, Frombus, 1 / R
                )  # ft_idx should not add, if this does not exist, it will be created
                temp = [Frombus, Tobus, Req, icf, Gv1, R, L, C, 0]
            else:
                ini_ft.addtoG0(
                    ft_idx, Frombus, Frombus, 1 / R + 1 / (Rs + emt.ts / 2 / C)
                )
                temp = [
                    Frombus,
                    Tobus,
                    Rs + emt.ts / 2 / C,
                    -(emt.ts / 2 / C - Rs) / (emt.ts / 2 / C + Rs),
                    -1 / (emt.ts / 2 / C + Rs),
                    R,
                    L,
                    C,
        ...
        for j in indx_ft:
            if (
                np.abs(ini.Init_net_coe0[j, 0] - Frombus)
                + np.abs(ini.Init_net_coe0[j, 1] - Tobus)
                < 1e-5
            ):
    ...
    for i in range(len(pfd.gen_bus)):
        ...
        ini_ft.addtoG0(
            ft_idx + 1, genbus_idx, genbus_idx + N1, ini.Init_mac_Gequiv[i, 0, 1]
        )
        ini_ft.addtoG0(
            ft_idx + 2, genbus_idx, genbus_idx + N2, ini.Init_mac_Gequiv[i, 0, 2]
        )
        ini_ft.addtoG0(
            ft_idx + 3, genbus_idx + N1, genbus_idx, ini.Init_mac_Gequiv[i, 1, 0]
        )
        ini_ft.addtoG0(
            ft_idx + 4, genbus_idx + N1, genbus_idx + N1, ini.Init_mac_Gequiv[i, 1, 1]
        )
        ini_ft.addtoG0(
            ft_idx + 5, genbus_idx + N1, genbus_idx + N2, ini.Init_mac_Gequiv[i, 1, 2]
        )
        ini_ft.addtoG0(
            ft_idx + 6, genbus_idx + N2, genbus_idx, ini.Init_mac_Gequiv[i, 2, 0]
        )
        ini_ft.addtoG0(
            ft_idx + 7, genbus_idx + N2, genbus_idx + N1, ini.Init_mac_Gequiv[i, 2, 1]
        )
        ini_ft.addtoG0(
            ft_idx + 8, genbus_idx + N2, genbus_idx + N2, ini.Init_mac_Gequiv[i, 2, 2]
        )
        ft_idx += 9
    ...
    for i in range(len(pfd.gen_bus)):
        ...
        ini_pf.addtoG0(
            pf_idx + 1, genbus_idx, genbus_idx + N1, ini.Init_mac_Gequiv[i, 0, 1]
        )
        ini_pf.addtoG0(
            pf_idx + 2, genbus_idx, genbus_idx + N2, ini.Init_mac_Gequiv[i, 0, 2]
        )
        ini_pf.addtoG0(
            pf_idx + 3, genbus_idx + N1, genbus_idx, ini.Init_mac_Gequiv[i, 1, 0]
        )
        ini_pf.addtoG0(
            pf_idx + 4, genbus_idx + N1, genbus_idx + N1, ini.Init_mac_Gequiv[i, 1, 1]
        )
        ini_pf.addtoG0(
            pf_idx + 5, genbus_idx + N1, genbus_idx + N2, ini.Init_mac_Gequiv[i, 1, 2]
        )
        ini_pf.addtoG0(
            pf_idx + 6, genbus_idx + N2, genbus_idx, ini.Init_mac_Gequiv[i, 2, 0]
        )
        ini_pf.addtoG0(
            pf_idx + 7, genbus_idx + N2, genbus_idx + N1, ini.Init_mac_Gequiv[i, 2, 1]
        )
        ini_pf.addtoG0(
            pf_idx + 8, genbus_idx + N2, genbus_idx + N2, ini.Init_mac_Gequiv[i, 2, 2]
        )
        pf_idx += 9
    ini_pf.Init_net_G0 = sp.coo_matrix(
        (ini_pf.Init_net_G0_data, (ini_pf.Init_net_G0_rows, ini_pf.Init_net_G0_cols)),
        shape=(ini_pf.Init_net_N, ini_pf.Init_net_N),
    ).tolil()
